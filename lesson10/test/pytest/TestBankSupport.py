...
def test_no_card(account_provider, card_provider):
    ...
    assert card_kind== card_provider.NO_CARD, "Incorrect type of card has been returned: {}. Should be {}".format(card_kind, card_provider.NO_CARD)

def test_normal_card(account_provider, card_provider):
    card_kind = account_provider(1).card_kind
    assert card_kind== card_provider.NORMAL_CARD, "Incorrect type of card has been returned: {}. Should be {}".format(card_kind, card_provider.NORMAL_CARD)

def test_gold_card(account_provider, card_provider):
    card_kind = account_provider(10001).card_kind
    assert card_kind== card_provider.GOLD_CARD, "Incorrect type of card has been returned: {}. Should be {}".format(card_kind, card_provider.GOLD_CARD)
...
def test_negative_debet(account_provider, check_exception):
    negative_debet = -10.0
    assert check_exception(account_provider, IncorrectDebetError, negative_debet), "Exception IncorrectDebetError hasn't been raised when created account with negative debet {}".format(str(negative_debet))
